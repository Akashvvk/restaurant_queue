...
LOG_FILE = "whatsapp_bot_log.txt"

# Define the secret message expected from QR code scans for customer initiation
QR_CODE_SECRET_MESSAGE = "dljflkasjdflkfj" # This should be kept secret and only in your QR code
...
def log_message(direction, sender_or_recipient, message_content, message_type="text"):
    """
    Logs incoming or outgoing messages to a file with a timestamp.

    Args:
        direction (str): "INCOMING" or "OUTGOING".
        sender_or_recipient (str): The phone number of the sender (for incoming)
                                   or recipient (for outgoing).
        message_content (str): The actual text content of the message.
        message_type (str): The type of message (e.g., "text", "image", etc.).
    """
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    log_entry = f"[{timestamp}] {direction} [{sender_or_recipient}] ({message_type}): {message_content}\n"
    try:
...
def send_message(to, text):
    ...
    try:
        ...
        log_message("OUTGOING", to, text) # Log outgoing message
        return response.json()
    except requests.exceptions.RequestException as e:
        print(f"Error sending message: {e}")
        log_message("OUTGOING_FAILED", to, f"Error: {e} - Original Text: {text}") # Log failed outgoing
        return {"error": str(e)}
        ...
        for entry in data["entry"]:
            for change in entry["changes"]:
                if change["field"] == "messages" and change["value"].get("messages"):
                    ...
                    # Log all incoming messages, regardless of type or if handled by bot logic
                    if message_type == "text":
                        message_content = msg["text"]["body"]
                        log_message("INCOMING", sender, message_content, message_type)
                    else:
                        # For non-text messages, log a generic representation
                        message_content = f"Non-text message received (type: {message_type})"
                        log_message("INCOMING", sender, message_content, message_type)
                    ...
                    if message_type == "text":
                        ...
                        # --- Customer Flow Logic ---
                        # Modified to accept "hi" OR the QR_CODE_SECRET_MESSAGE
                        elif user_state["state"] == "initial" and (text == "hi" or text == QR_CODE_SECRET_MESSAGE):
